...
dd_options.extend([{'label': label, 'value': label} 
                   for label in spacex_df['Launch Site'].unique().tolist()])
...
def get_scatter_plot_chart(entered_site, slider_range):
    low, high = slider_range
    mask = (spacex_df['Payload Mass (kg)'] >= low) & (spacex_df['Payload Mass (kg)'] <= high)
    filtered_df = spacex_df[mask]
    if entered_site == 'ALL':
        filtered_df = filtered_df[['Booster Version Category', 'Payload Mass (kg)', 'class']]
...
# Local:
# cd D:\dev\git\code\estudos\data_science\notebooks
